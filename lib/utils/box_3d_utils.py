...
def box_3d_to_anchor_torch(boxes_3d, ortho_rotate=False):
    ...
    x, y, z, l, h, w, ry = torch.split(boxes_3d, 1, dim=-1)
    ...
    anchors = torch.cat([x,y,z,dimx,dimy,dimz], dim=-1)
...
def get_box3d_corners_helper_torch(centers, headings, sizes):
    N = centers.shape[0]
    l = sizes[:, 0:1]  # (N,1)
    h = sizes[:, 1:2]  # (N,1)
    w = sizes[:, 2:3]  # (N,1)
    ...
    y_corners = torch.cat([z, z, z, z, -h, -h, -h, -h], dim=1)  # (N,8)
    ...
    ones = torch.ones([N]).to(headings.device)
    zeros = torch.zeros([N]).to(headings.device)
    ...
    corners_3d = corners_3d.transpose(1, 2)  # (N,8,3)
    return corners_3d


def transfer_box3d_to_corners_torch(boxes_3d):
    b, n = boxes_3d.shape[0], boxes_3d.shape[1]
    boxes_3d_reshape = boxes_3d.view(-1, 7)
    corners = get_box3d_corners_helper_torch(boxes_3d_reshape[:, :3], boxes_3d_reshape[:, -1], boxes_3d_reshape[:, 3:-1])
    corners = corners.view(b, n, 8, 3)
    return corners
